...
public class AdminMessageKafka {
    ...
    @GetMapping("/topics/{topic}/messages-flagged")
	@Operation(summary = "Lista mensajes por Topic marcados como leidos", description = "Lista mensajes por Topic marcados como leidos")
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "OK", content = {
					@Content(mediaType = "application/json") }),
			@ApiResponse(responseCode = "400", description = "Solicitud inválida", content = {
			...
			@ApiResponse(responseCode = "500", description = "Error interno del servidor", content = {
					@Content(mediaType = "application/json") }) })
    public List<Map<String, Object>> getMessagesBeforeOffset(
    		@PathVariable String topic,
            @PathVariable int partition,
            @RequestParam int offset,
            @RequestParam int limit){
        return iKafkaRelaunchMessage.getMessagesBeforeOffset(topic, partition, offset, limit);
    }
